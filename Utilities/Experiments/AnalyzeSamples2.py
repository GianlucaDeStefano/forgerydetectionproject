...
class AnalyzeSamples(Logger):
    ...
    def process(self):
        ...
        for sample_path in tqdm(self.get_samples_to_process()):
            ...
            sample_debug_root = self.configs.create_debug_folder(sample_name.split('.')[0])

            self.logger_module.info(f"###{sample_name}###")

            original_forgery_mask = self.get_original_forgery_mask(sample_path)

            target_forgery_mask = None
            if self.is_decoy_attack:
            ...
            target_forgery_mask2_print = target_forgery_mask
            if not self.is_decoy_attack:
                target_forgery_mask2_print = np.zeros(original_forgery_mask.shape)

            complete_analysis_visualization(Picture(sample_path), original_forgery_mask, original_heatmap,
                                            target_forgery_mask2_print, attacked_heatmap,
    ...
    def compute_detection_rates(self, gt_mask, decoy_mask, combined_mask, predicted_mask):
        ...
        assert gt_mask.shape == combined_mask.shape == predicted_mask.shape

        dr_gt = ((gt_mask + predicted_mask) == 2).sum() / gt_mask.sum()

        dr_decoy = 0
        if self.is_decoy_attack:
            dr_decoy = ((decoy_mask + predicted_mask) == 2).sum() / decoy_mask.sum()

        dr_bg = ((combined_mask + predicted_mask) == 2).sum() / (combined_mask == 0).sum()
    ...
    def compute_thresholding_metrics(self, heatmap_pristine, heatmap_attacked, original_forgery_mask,
        ...
        combined_mask = original_forgery_mask > 0
        ...
        if self.is_decoy_attack:
            ...
            dr_gt_f1_3, dr_decoy_f1_3, dr_bg_f1_3 = self.compute_detection_rates(original_forgery_mask,
                                                                                 target_forgery_mask,
    ...
    @property
    def is_decoy_attack(self):
        return Path(self.attacked_samples_target_forgery_masks_folder).exists()
...
def roc_curves_visualization(heatmap_pristine, original_forgery_mask, heatmap_attacked, target_forgery_mask, path):
    ...
    auc_dec_pre_fpr, auc_dec_pre_tpr, auc_dec_post_fpr, auc_dec_post_tpr = None,None,None,None

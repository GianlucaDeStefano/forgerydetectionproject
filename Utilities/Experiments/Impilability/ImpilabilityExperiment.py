...
class ImpilabilityExperiment(BaseExperiment):
    ...
    def __init__(self, attack: BaseWhiteBoxAttack, debug_root, dataset,attacked_samples_folder_path):
        ...
        self.target_forgery_masks_folder_path = os.path.join(self.attacked_samples_folder_path, "targetForgeryMasks")
    ...
    def process_sample(self, sample_path, gt_mask):
        ...
        # load the original forgery mask and enforce that its width and height are the same of the given sample by
        # resizing it
        original_forgery_mask = Picture(resize_mask(gt_mask, (pristine_sample.shape[1], pristine_sample.shape[0])))

        target_forgery_mask = Picture(path=os.path.join(self.target_forgery_masks_folder_path, filename))
        target_forgery_mask = np.rint(Picture(target_forgery_mask[:, :, 0]) / 255)

        # load the target forgery mask

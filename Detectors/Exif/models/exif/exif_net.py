...
from Detectors.Exif.nets import resnet_v2, resnet_utils
...
class EXIFNet():
    ...
    def __init__(self, num_classes=83, train_classifcation=False,
        ...
        if self.use_tf_threading:
            ...
            self.im_a = tf.compat.v1.placeholder_with_default(im_a, [None, self.im_size, self.im_size, 3])
            self.im_b = tf.compat.v1.placeholder_with_default(im_b, [None, self.im_size, self.im_size, 3])
            ...
            self.im_a = tf.compat.v1.placeholder(tf.float32, [None, self.im_size, self.im_size, 3])
            self.im_b = tf.compat.v1.placeholder(tf.float32, [None, self.im_size, self.im_size, 3])
        ...
        self.im_a_index = tf.compat.v1.placeholder(tf.int32, [None, ])
        self.im_b_index = tf.compat.v1.placeholder(tf.int32, [None, ])
    ...
    def model(self, preemptive_reuse=False):
                    ...
                    if self.train_classifcation:
                        with tf.name_scope('predict_same_image'):
                            if self.use_classify_with_feat:
                                ...
                                pc_cls_out = self.classify_with_feat(pc_im_a_feat, pc_im_b_feat, pc_out,
                                                                     name='classify',
                                                                     reuse=True)
                            else:
                        ...
                        with tf.name_scope('classification_loss'):
                            cls_loss = tf.reduce_mean(
                                tf.nn.sigmoid_cross_entropy_with_logits(labels=cls_label, logits=cls_out))
        ...
        if self.train_classifcation:
            ...
            self.cls_accuracy = tf.reduce_mean(
                tf.cast(tf.equal(tf.round(self.cls_pred), tf.round(self.cls_label)), tf.float32))
        return
        ...
        with tf.name_scope(name):
            with slim.arg_scope(resnet_utils.resnet_arg_scope()):
                ...
                if not use_global_pool:
                    args = {'reuse': reuse, 'norm': None, 'activation': tf.nn.relu, 'padding': 'SAME',

...
from Datasets.Utilities.Noiseprint.noiseprint import gen_noiseprint
from Datasets.Utilities.Noiseprint.utility import noiseprint2pnd
from Datasets.Utilities.utilityFunctions import get_files_with_type

"Class to download the CASIA2 dataset"


class CASIA2Noiseprint(tfds.core.GeneratorBasedBuilder):
    ...
    def __init__(self, **kwargs):
        ...
        super(CASIA2Noiseprint, self).__init__()
        ...
        #print only tensorflow errors and not warnings
    ...
    def features(self):
        ...
        """
        return {
            'image': tfds.features.Image(shape=(256, 384, 3)),
            'noiseprint':  tfds.features.Tensor(shape=(256,384,1), dtype=tf.float32),
            'mask': tfds.features.Image(shape=(256, 384, 1)),
    ...
    def _generate_examples(self, authentic_files: list, tampered_files: list):
        ...
        print("Generated dataset containing: {} authentic and {} tampered images".format(counter_authentic,
                                                                                         counter_tampered))
    ...
    def _keep_valid(self, files_paths: list):
        ...
        for file_path in files_paths:
            ...
            # for the moment we just use images 384x256 or 256x384 to make the train easier
            if image.shape[0] not in supported_dims or image.shape[1] not in supported_dims:
                continue
    ...
    def _process_image(self, path, mask_path=None):
        ...
        assert(image.shape[0] == target_shape[0])
        assert (image.shape[1] == target_shape[1])

        #generate the noiseprint from the given image
        noiseprint = gen_noiseprint(path)[..., np.newaxis]
        ...
        return {"image": image,"noiseprint":noiseprint,"mask": mask.astype(np.uint8),"flipped":flipped, "tampered": tampered}
